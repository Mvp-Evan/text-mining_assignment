...
class LSTM_UP(nn.Module):
    def __init__(self, config):
        ...
        self.vae = VAE(Encoder(input_size, hidden_size, 500), Decoder(500, hidden_size, input_size))

        self.rnn = EncoderLSTM(input_size, hidden_size, 1, True, False, 1 - config.keep_prob, False)
        self.attention = BiAttention(hidden_size, 1 - config.keep_prob)
        self.linear_re = nn.Linear(512, 128)  # *4 for 2layer
...
def reparameterization(mean, var):
        epsilon = torch.randn_like(var).to(torch.device("mps"))
